...
def delete_everything_inside_folder(folder_path, to_keep=None):
...
def delete_empty_folder(folder_path):
    if not os.path.isdir(folder_path):
        raise Exception(f"not a folder: {folder_path = }")
    if (stuff:=os.listdir(folder_path)):
        raise Exception(f"folder is not empty: {stuff}")
    if os.path.isdir(folder_path):
        shutil.rmtree(folder_path)
        return
    raise Exception("handle that")
...
def fetch_file_stream(url, destination_folder_path, data_file_name, redownload=False):
    downloaded_file_path = os.path.join(destination_folder_path, data_file_name)
    if not redownload:
...
def unzip_files(zipped_file_path, destination_folder_path, reextract=False):
